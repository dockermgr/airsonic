...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202307051220-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.com
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Wednesday, Jul 05, 2023 13:16 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for airsonic
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/airsonic"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="airsonic"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/airsonic"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/airsonic"
...
HUB_IMAGE_URL="casjaysdevdocker/airsonic"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [airsonic.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
CONTAINER_WEB_SERVER_INT_PORT="4040"
...
CONTAINER_ADD_CUSTOM_PORT="0.0.0.0:4041:4041 0.0.0.0:1900:1900/udp"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_MOUNTS="$HOME/Music:/var/music/$USER:z,$DATADIR/data/music:/var/music:z "
CONTAINER_MOUNTS+="$DATADIR/data/podcasts:/var/podcasts,$DATADIR/data/playlists:/var/playlists:z"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_ENV="ADDITION_ENV=JAVA_OPTS=-Dserver.forward-headers-strategy=native"
CONTAINER_ENV+=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set sysctl - []
CONTAINER_SYSCTL=""
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-airsonic}}"
...
if [ -n "$AIRSONIC_USERNAME" ]; then
  CONTAINER_USER_NAME="$AIRSONIC_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${AIRSONIC_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$AIRSONIC_PASSWORD" ]; then
  CONTAINER_USER_PASS="$AIRSONIC_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${AIRSONIC_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
