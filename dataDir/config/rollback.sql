...
-- Ran at: 11/12/21, 1:13 PM
-- Against: SA@jdbc:hsqldb:file:/var/airsonic/db/airsonic;hsqldb.tx=mvcc;sql.enforce_size=false;sql.char_literal=false;sql.nulls_first=false;sql.pad_space=false;hsqldb.defrag_limit=50;shutdown=true
...
-- Lock Database
UPDATE PUBLIC.DATABASECHANGELOGLOCK SET LOCKED = TRUE, LOCKEDBY = 'airsonic.local (172.17.0.4)', LOCKGRANTED = '2021-11-12 13:13:11.645' WHERE ID = 1 AND LOCKED = FALSE;
...
CREATE TABLE PUBLIC.player_transcoding (player_id INT NOT NULL, transcoding_id INT NOT NULL, CONSTRAINT pt_p_fk FOREIGN KEY (player_id) REFERENCES PUBLIC.player(id) ON DELETE CASCADE, CONSTRAINT pt_t_fk FOREIGN KEY (transcoding_id) REFERENCES PUBLIC.transcoding(id) ON DELETE CASCADE);
